...
class EventManager {
    ...
    public function clearEventListeners($event = null) {
    	...
    	if (!is_string($event) || $event == '') {
    		throw new EventException('Could not clear the event listeners: provided event name is invalid or empty');
    	}
     ...
     */
    public function unregisterEventListener($eventListener, $event = null) {
    	$isListener = $eventListener instanceof EventListener;
    	if ($isListener) {
    		$event = $eventListener->getEvent();
    	} elseif (!is_numeric($eventListener)) {
    		$eventListener = new Callback($eventListener);
    	}
    ...
    public function triggerEvent($event, array $arguments = null) {
        ...
        foreach ($this->events[$event->getName()] as $eventListener) {
        	$callback = $eventListener->getCallback();
